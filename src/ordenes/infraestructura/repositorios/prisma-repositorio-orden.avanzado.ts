...
import { Orden, DatosClienteSnapshot, DatosEnvioSnapshot, DatosFacturacionSnapshot } from '../../dominio/entidades/orden.entity';
...
/**
 * Repositorio de Orden avanzado implementado con Prisma
 * Incluye todas las funcionalidades Shopify-like
 */
@Injectable()
export class PrismaRepositorioOrdenAvanzado implements RepositorioOrden {
  ...
  private aEntidad(prismaOrden: any): Orden {
    return new Orden(
      ...
      prismaOrden.datos_cliente as DatosClienteSnapshot | null,
      prismaOrden.datos_envio as DatosEnvioSnapshot | null,
      prismaOrden.datos_facturacion as DatosFacturacionSnapshot | null,
  ...
  async listar(
    ...
    // Filtros avanzados
    if (filtros?.esBorrador !== undefined) {
      where.es_borrador = filtros.esBorrador;
    }

    if (filtros?.archivada !== undefined) {
      where.archivada = filtros.archivada;
    }

    if (filtros?.soloBorradores) {
    ...
    if (filtros?.incluirAbandonadas === false) {
      where.fecha_abandono = null;
    }

    if (filtros?.soloAbandonadas) {
      where.fecha_abandono = { not: null };
    }

    // Filtros de fecha de abandono
    if (filtros?.fechaAbandonoDesde || filtros?.fechaAbandonoHasta) {
      where.fecha_abandono = {};
      
      if (filtros.fechaAbandonoDesde) {
        where.fecha_abandono.gte = filtros.fechaAbandonoDesde;
      }
      
      if (filtros.fechaAbandonoHasta) {
        where.fecha_abandono.lte = filtros.fechaAbandonoHasta;
      }
    }
  ...
  async duplicarOrden(ordenId: string, creadorId: string): Promise<Orden> {
    ...
    const nuevoNumeroOrden = await this.generarNumeroOrdenUnico();

    // Crear nueva orden duplicada
    const nuevaOrden = new Orden(
      ...
      ordenOriginal.notas,
      `Orden duplicada de ${ordenOriginal.numeroOrden}`,
      new Date(),
      new Date(),
      null, // fechaPago
      null, // fechaEnvio
  ...
  async crearReposicion(ordenId: string, creadorId: string): Promise<Orden> {
    ...
    // Crear nueva orden de reposición
    const nuevaOrden = new Orden(
      this.generarId(),
      nuevoNumeroOrden,
      ordenOriginal.clienteId,
